...
### 新增 (Added)
- ✨ **任务置顶功能**:
  - 新增 `is_pinned` 字段到任务数据模型
  - 任务卡片右上角新增 📌 置顶按钮
  - 视觉反馈：未置顶灰色半透明 → 已置顶彩色旋转45度
  - 置顶任务永远排在列表最前面

- ✨ **智能权重排序系统**:
  - 重构排序逻辑为权重计算模型
  - 权重分层：置顶区(0-2) → 逾期高危区(100-200) → 逾期衰减区(200-500) → 紧急预警区(600-650) → 正常待办区(700-900) → 已完成区(10000+)
  - 置顶区内部按优先级细分（高/中/低）
  - 逾期任务惩罚衰减机制（3天阈值）
  - 2小时紧急预警自动提升权重
...
### 优化 (Improved)
- 🎯 **排序逻辑优化**:
  - 解决逾期任务"淹没首屏"问题（逾期3天以上低优先级任务降权）
  - 解决"紧急但不逾期"任务被忽略问题（2小时内到期自动提升）
  - 解决置顶任务"通货膨胀"问题（置顶区内部严格按优先级排序）
  - 基于截止时间动态计算权重，避免视觉噪音

### 技术细节 (Technical)
- 📦 Store层：新增 `togglePin()` 方法
- 📦 Store层：重构 `getFilteredTasks()` 排序算法
- 🎨 UI层：任务卡片新增 `.task-actions` 按钮组
- 🎨 UI层：新增 `.btn-pin-inline` 样式（灰度/彩色/旋转状态）
- 📄 文档：新增 `SORTING_UPGRADE.md` 排序系统升级说明
