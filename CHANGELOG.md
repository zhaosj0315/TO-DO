...
### 新增 (Added)
- 🛠️ **编辑器工具栏组件化**：抽取 `FullscreenDescEditor.vue` 全屏描述编辑器组件，5个工具统一复用
- 📝 **任务详情描述区全屏编辑**：新增 ⛶ 最大化按钮，支持插入图片和文件附件，关闭后自动保存
- 📋 **添加日志全屏编辑与 Markdown 预览**：日志内容区新增 ⛶ 最大化和 👁️ 预览切换，支持 Markdown 渲染
...
### 优化 (Changed)
- 🗂️ **Header 布局重构**：第二行（标签/图谱/甘特图/日历）默认收起，点击"更多"展开
...
### 新增 (Added)
- 📌 节点拖动固定（gravity 降低 + mouseup 固定位置）
- 📖 教程模式更新：v0.9.2 全面版（18 步）

### 优化 (Changed)
- 🕸️ 节点显示优化：圆圈内显示关系数量，圆圈下显示任务名称
- 🔍 双击聚焦功能：双击节点展开其直接关系网络
...
### 新增 (Added)
- ✨ **Obsidian 风格任务关系系统**：双向链接 `[[任务名]]`、层级标签 `#work/project-a`、反向链接面板、自动补全
- 🏷️ **标签浏览器**：树形结构展示所有标签，点击筛选，Bottom Sheet 布局
- 🕸️ **任务关系图谱**：ECharts 可视化，节点拖动，关键字搜索高亮
- 📊 **甘特图视图**：时间轴可视化，周/月/季度视图，优先级颜色
- 📅 **日历视图**：月视图42天网格，优先级标记，任务下钻
- 🔙 **Android 返回手势全面支持**：标签浏览器、图谱、甘特图、日历视图

### 优化 (Changed)
- 📊 配额管理优化：报告历史三重防护，回收站自动清理
- 💾 备份提醒按用户隔离，首次登录只提醒一次
...
### 新增 (Added)
- ✨ **富媒体附件系统（Phase 2）**：支持图片/PDF/Word/Excel/PPT/视频/压缩包等8大类型
- 👁️ **文件预览系统**：全屏预览弹窗，支持图片/PDF/视频

### 优化 (Changed)
- 🎨 AI 助手和报告中心统一使用 MarkdownRenderer 组件渲染
- 🎯 任务排序逻辑全面重构：时间紧迫性优先，连续权重体系
- 📊 AI问答时间维度修复：完成类问题看 `completed_at`

### 修复 (Fixed)
- 🐛 修复任务状态丢失问题（刷新后完成状态重置）
- 🐛 修复 Android 返回手势失效
...
### 新增 (Added)
- 📝 **Markdown 编辑器**：双模式切换（编辑/预览），GitHub 风格，DOMPurify 安全防护
...
- ✨ **AI模型配置全面重构**:
  ...
  - 统一厂商选择：已保存配置和预设厂商合并到一个下拉框
  - 智能获取模型：输入API Key后自动获取模型列表
  - 分组显示：按类型分组（已保存/本地/官方/中转/国产/云/开源）
  - 零学习成本：无需手动输入地址，选择厂商自动填充

### 修复 (Fixed)
- 🐛 **关键Bug修复**:
  - 修复Android返回手势失效：删除未定义的变量引用
  - 修复孤儿笔记本问题：自动检测并修复父笔记本不存在的笔记本
  - 修复批量删除递归：删除笔记本时自动递归删除所有子笔记本

### 优化 (Changed)
- 🎨 **UI布局优化**:
  - 统计栏重构：删除"全部"按钮，避免功能重复
  - 胶囊按钮样式统一：笔记本和统计栏改为横向胶囊布局
  - 间距极致紧凑：所有行间距统一为2px
  - Header图标重排：按使用频率排序
