...
- ✨ **文件预览系统** 👁️:
  ...
  - **多格式预览支持**：
    - 📸 **图片**：全屏居中显示，黑色背景，点击图片直接预览
    - 📕 **PDF**：浏览器原生 iframe 预览，支持缩放和翻页
    - 🎬 **视频**：HTML5 播放器，支持 MP4、MOV、AVI 等格式
    - 📄 **文本**：等宽字体显示，保留格式（TXT、RTF）
  - **优雅降级**：不支持预览的文件显示友好提示和下载按钮
  - **一键下载**：所有文件支持点击下载到本地
- 🎨 **用户体验优化**:
  - **图片默认尺寸**：300px 宽度显示，避免占满屏幕
  - **文件卡片设计**：显示文件图标、名称、大小，悬停上浮效果
  - **点击交互优化**：任务详情页预览模式只在空白区域点击切换编辑
  - **事件绑定修复**：解决 DOMPurify 移除事件处理器的问题
- 🔧 **技术实现**:
  - **Store 扩展**：addTask 方法支持 media 字段保存
  - **渲染器增强**：MarkdownRenderer 支持图片和文件混合渲染
  - **预览组件**：新增 FilePreviewModal 组件，支持多种文件格式
  - **文件类型判断**：智能识别 20+ 种文件扩展名
  - **Base64 处理**：文件转换和 Blob URL 生成
- 📊 **支持的文件类型**:
  - **图片**：JPG、PNG、GIF、BMP、WebP
  - **文档**：PDF、Word (DOC/DOCX)、TXT、RTF
