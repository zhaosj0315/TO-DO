  ...
  const allVersions = [
    {
      ...
      features: [
        ...
        '📂 展开/收起功能：支持查看完整版本历史，避免信息过载',
        '🔙 Android返回手势全面修复：支持47个弹窗的手势导航和按钮导航'
      ],
      improvements: [
        '📊 历史报告视觉提示：黄色提示条显示报告生成时间（如：2026-03-05 21:30）',
        '💾 智能按钮切换：新报告显示"保存"，历史报告显示"另存为"',
        '🔒 禁用报告类型切换：历史模式下禁止修改报告类型，保护数据完整性',
        '📋 版本描述精简：每个版本只保留3-5个核心特性，提升可读性',
        '🎯 版本号连贯性：补全0.7.12/0.7.9/0.7.0/0.6.12/0.5.0/0.3.0/0.1.0关键版本',
        '🎨 Header区域优化：5个功能图标右对齐，整体放大1.5倍（42px）',
        '📱 右上角图标间距：从1rem缩小到0.5rem，更紧凑',
        '👤 个人中心按钮：显示用户名，紫色渐变背景+白色边框',
        '✨ 视觉统一：所有按钮统一紫色渐变+白色边框+一致交互效果',
        '📊 统计卡片优化：文字统一为两个字（完成、逾期），字体放大',
        '📝 任务卡片布局：描述和属性行左对齐到复选框，充分利用空间',
        '🔘 任务操作按钮间距：从0.25rem增加到0.8rem，避免误触',
        '🍅 任务卡片精简：删除番茄数量显示，只在任务详情页保留',
        '🤖 AI问答界面：展开/收起按钮移至左上角，收起时全屏显示对话'
      ],
      fixes: [
