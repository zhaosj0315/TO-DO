  ...
  const allVersions = [
    ...
    },
    {
      ...
      improvements: [
        ...
        '🤖 AI配置读取：支持新格式ai_models和旧格式ai_model_config',
        '📝 报告结构优化：4部分（概述/成果/进展/建议），700-900字',
        '🎯 智能报告中心：z-index提升到10008，AI汇报100001',
        '📊 完成率修复：100%显示正常（之前显示100%%）'
      ],
      fixes: [
        '修复合并笔记本层级丢失：完整保留子笔记本结构',
        '修复AI配置读取失败：localStorage key不匹配（ai_model_config vs ai_models）',
        '修复Ollama URL拼接错误：/api/generate → /v1/chat/completions',
        '修复智能报告中心被遮挡：z-index从2000提升到10008',
        '修复AI汇报弹窗被遮挡：z-index提升到100001',
        '修复完成率重复百分号：100%% → 100%',
        '修复首页返回手势无响应问题',
        '修复演示功能步骤过多（30→24步）'
      ]
    },
    {
      ...
      features: [
        ...
        '🎨 版本历史优化：精简为10个关键里程碑版本，默认显示最近3个',
        '🔙 Android返回手势全面修复：支持48个弹窗的手势导航和按钮导航'
      ],
      improvements: [
        '🌲 任务树设计：圆形图标（42px），绿色渐变，呼吸动画，点击查看详情',
        '📊 成长值计算：完成任务+1分，高优先级+2分，番茄钟+0.5分',
        '🎯 10级成长体系：种子(0-79)→幼苗→树苗→小树→大树→开花→繁茂→茂盛→参天→神树(10000+)',
        '📈 成长详情弹窗：显示当前等级、成长值、进度条、等级说明、获取规则',
        '💾 智能按钮切换：新报告显示"保存"，历史报告显示"另存为"',
        '📋 版本描述精简：每个版本3-5个核心特性，提升可读性',
        '🎨 Header布局优化：左侧树+右侧5个图标，间距紧凑（0.25rem）',
        '✨ 视觉统一：所有按钮42px圆形，紫色渐变+白色边框',
        '📊 统计卡片优化：文字统一为两个字（完成、逾期），字体放大',
        '🔘 任务操作按钮间距：从0.25rem增加到0.8rem，避免误触'
      ],
      fixes: [
        '修复版本历史0.0.0→0.7.x跳跃问题',
        '修复返回手势在手势导航设备上不工作',
        '修复统计中心返回时跳过个人主页',
        '修复UnifiedReportModal初始化错误导致崩溃',
        '修复任务树详情弹窗缺少返回手势支持'
      ]
    },
    {
...
const treeLevel = computed(() => {
  ...
  if (score >= 10000) return 10  // 神树
  if (score >= 5000) return 9    // 参天大树
  if (score >= 3000) return 8    // 茂盛大树
  if (score >= 2000) return 7    // 繁茂之树
  if (score >= 1200) return 6    // 开花结果
  if (score >= 700) return 5     // 大树
  if (score >= 400) return 4     // 小树
  if (score >= 200) return 3     // 树苗
  if (score >= 80) return 2      // 幼苗
  return 1                       // 种子
})
...
const treeProgress = computed(() => {
  const score = growthScore.value
  const thresholds = [0, 80, 200, 400, 700, 1200, 2000, 3000, 5000, 10000, 999999]
...
const getNextLevelScore = () => {
  const thresholds = [80, 200, 400, 700, 1200, 2000, 3000, 5000, 10000, 999999]
