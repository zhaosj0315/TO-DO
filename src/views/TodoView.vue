  ...
  const allVersions = [
    {
      ...
      features: [
        ...
        '  • AI助手内置8个功能：生成标题/描述/续写/润色/提取要点/改写/Markdown渲染/自动打标签',
        '📝 任务详情描述区支持全屏编辑：',
        ...
        '  • 全屏编辑器关闭后自动切换到预览模式',
        '🏷️ AI 自动打标签（三处入口统一支持）：',
        '  • 创建任务全屏编辑器 / 任务详情描述区 / 添加日志均支持',
        '  • AI 根据标题+描述推荐 3-6 个 #标签，结果以 chip 形式展示',
        '  • 支持点击删除不需要的标签，点击「+ 添加」新增自定义标签',
        '  • 采纳后标签追加到描述末尾，自动进入标签浏览器体系',
        '📓 AI 智能归类未分类任务（双入口）：',
        '  • 批量迁入弹窗新增「🤖 AI 推荐分类」：针对单个笔记本推荐任务并自动勾选',
        '  • 笔记本管理新增「🤖 AI 智能归类」：一次性分析所有未分类任务',
        '  • 支持推荐归入现有笔记本或建议新建笔记本，确认后自动创建并批量归类',
      ],
      improvements: [
        '📎 日志附件支持：日志数据结构新增 media 字段，附件随日志持久化保存',
        '🔄 任务详情日志列表：MarkdownRenderer 传入 media，图片和文件正确渲染',
        '💾 添加日志后自动刷新任务详情，确保 logs 和 media 数据同步',
        '🎯 Header 第一行图标顺序优化：AI助手→笔记本→刷新→回收站，右侧教程→更多→我的主页',
        '📊 甘特图全面升级：时间轴移至顶部，支持日/周/月/季/年视图切换',
        '📊 甘特图时间范围自动包含所有任务，切换视图不丢失任务',
        '📊 甘特图任务按截止时间升序排列，最近到期的显示在顶部',
        '📊 甘特图截止时间以任务创建时间为基准推算（而非当前时间）',
        '📊 甘特图已完成任务右端改用实际完成时间（completed_at）',
        '📊 甘特图 tooltip 新增计划完成时间 vs 实际完成时间对比',
        '📊 甘特图时间轴最小宽度保证：周≥3周/月≥3月/季≥3季/年≥3年',
      ...
      fixes: [
        ...
        '🐛 甘特图：修复截止时间以当前时间推算导致历史任务显示错误',
        '🐛 甘特图：修复已完成任务条形右端未使用实际完成时间',
        '🐛 甘特图：修复时间轴过短导致任务条形被压缩',
        '🐛 甘特图：修复 slice 取前N条导致最近截止任务被截断',
        '🐛 甘特图：修复 daily/weekday 重复任务条形越来越长（固定1天宽）',
        '🐛 甘特图：修复 weekly 任务当天恰好是目标星期几时条形极窄',
        '🐛 甘特图：所有条形保证最小1小时宽，避免极窄条形',
        '🐛 图谱：修复 relationStats 用自环边判断日志关系（改为颜色判断）',
        '🐛 图谱：修复 calcRelCount 未计入日志关系数导致排序权重偏低',
        '🐛 图谱：修复 BFS 邻居扩展未包含标签关系和笔记本关系',
        '🐛 图谱：新增笔记本同组关系（📓 黄色点线），统计栏显示笔记本关系数',
