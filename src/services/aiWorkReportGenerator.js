...
2. 结构清晰，分为4个部分：
   ...
   **二、重点成果**（200-250字）
   - 列举3-5项最重要的成果（使用1. 2. 3.编号）
   - 每项成果说明：做了什么 + 产生了什么价值/影响
   - 按重要性排序
...
- 语言专业、简洁、有洞察力`
    ...
    try {
      ...
      // 添加报告元数据
      const reportHeader = `**${typeLabels[type] || '报告'}**\n**报告周期：${data.period.start} - ${data.period.end}**\n**生成时间：${new Date().toLocaleString('zh-CN')}**\n**AI模型：${this.aiConfig.model}**\n\n---\n\n`
      
      return reportHeader + result.choices[0].message.content
    } catch (error) {
