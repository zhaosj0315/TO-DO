# 文档导航 | Documentation Guide

本项目包含多个文档，本指南帮助您快速找到所需信息。

---

## 📚 核心文档

### [README.md](./README.md)
**用途**: 项目总览和快速开始 (v1.5.5)
**内容**: 项目简介、核心功能、技术栈、安装指南。

---

### [CHANGELOG.md](./CHANGELOG.md)
**用途**: 版本变更记录  
**内容**: 记录从 v1.0.0 到 v1.4.0 的所有功能更新。

---

### [CHANGELOG_v1.5_COMPLETE.md](./CHANGELOG_v1.5_COMPLETE.md)
**用途**: v1.5系列完整更新日志 ⭐ NEW
**内容**: v1.5.0-v1.5.5 的详细UI/UX重构记录。

---

### [FEATURES.md](./FEATURES.md)
**用途**: 完整功能说明 (v1.5.5)
**内容**: 任务类型、截止时间、番茄钟、筛选功能、添加表单、右上角按钮组。

---

### [USER_MANUAL.md](./USER_MANUAL.md)
**用途**: 用户使用手册  
**内容**: 包含番茄钟系统、数据导入导出、账户安全的操作指南。

---

## 🛠️ 技术文档

### [DEVELOPER.md](./DEVELOPER.md)
**用途**: 开发者指南  
**内容**: 架构概览、代码结构、本地存储 (Preferences) 逻辑、跨平台打包。

---

### [APK_BUILD_GUIDE.md](./APK_BUILD_GUIDE.md)
**用途**: APK打包详细指南  
**内容**: 完整的打包流程、Java 17 环境要求、修复脚本说明。

---

### [APK_BUILD_QUICK.md](./APK_BUILD_QUICK.md)
**用途**: APK打包快速参考  
**内容**: 快速命令列表。

---

## 🔍 审计报告

### [DOC_AUDIT_REPORT_20260220.md](./DOC_AUDIT_REPORT_20260220.md)
**用途**: v1.5.5版本文档审计报告（当前）⭐ NEW
**内容**: 文档-代码一致性检查、问题清单、修复计划。

---

### 历史审计报告
- [AUDIT_REPORT_V1.4.0.md](./AUDIT_REPORT_V1.4.0.md) - v1.4.0审计
- [AUDIT_REPORT_V1.3.1.md](./AUDIT_REPORT_V1.3.1.md) - v1.3.1审计
- [AUDIT_REPORT_V1.3.0.md](./AUDIT_REPORT_V1.3.0.md) - v1.3.0审计
- [AUDIT_REPORT.md](./AUDIT_REPORT.md) - 早期审计

---

**维护者**: External Auditor  
**最后更新**: 2026-02-20  
**文档版本**: v1.5.5
